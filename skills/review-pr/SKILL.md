...
**Rank the remedy before you write it.** When the finding is "these N sites must agree" — a predicate with several callers, a constant duplicated across modules, a contract two serializers both emit — the remedies are not equivalent:

1. **Unrepresentable** — change a type, signature, or shape so the wrong call cannot be written. No new code, no runtime cost.
2. **Fails loudly** — a test or assertion that catches the miss after someone writes it.
3. **Documented** — a comment stating the invariant.

Name the rung you're proposing and why not the one above. Reaching for a test is the default, and it is usually rung 2 where rung 1 was available: a stringly-typed parameter that should be an enum, an optional field that should be required, two shapes that should be one. A comment asserting an invariant is rung 3, and is itself a finding.
