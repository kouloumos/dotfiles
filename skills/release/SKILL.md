---
name: release
description: Merge source branch (default main) to production, create a CalVer release, and generate release content (GitHub release notes, Discord announcement, blog post)
...
Generate three markdown outputs, each following its template. Read each template before generating:
...
3. **Blog post** — see [templates/blog-post.md](templates/blog-post.md)

Present all three outputs to the user for review before proceeding.
...
5. Print the Discord and blog post markdown for the user to copy.
...
- The blog post is optional — ask the user if they want one. Small releases may not warrant a post.
